...
public abstract class Rating {
    ...
    public long inversions() {
        ...
        Player[] playersClone = new Player[players.length];
        System.arraycopy(players, 0, playersClone, 0, players.length);

        Arrays.sort(playersClone, BY_SKILL);
        Arrays.sort(playersClone, BY_RATING);

        for (int i = 0; i < n; i++) {
            multiset.add(playersClone[i].skill());
            inversions += multiset.backrank(playersClone[i].skill());
        }
    ...
    public long weightedInversions() {
        ...
        for (int i = 0; i < n; i++) {
            multiset.add(playersClone[i].skill());
            Iterable<Integer> q = multiset.keys(playersClone[i].skill() + 1, multiset.max());
            for (int skill : q) {
                weightedInversions += (skill - playersClone[i].skill());
            }
        }
