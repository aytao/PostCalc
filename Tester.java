...
public class Tester {
    ...
    public static void simulateInput(Elo elo, PostCalc postCalc, int rounds) {
        ...
        StdOut.printf("Average pair difference: %.3f\n", elo.avgSkillDiff());
        ...
        StdOut.println("\nAverage Ratings");
        StdOut.println("Elo: " + elo.avgRating());
        StdOut.println("PostCalc: " + postCalc.avgRating());
    }
    ...
    public static void saveRS(String fileName, Player[] players) {
        ...
        for (int i = 0; i < players.length; i++) {
            str.append(players[i].rating()).append(",");
            str.append(players[i].skill()).append("\n");
        }
    ...
    public static void main(String[] args) {
        ...
        saveRS("NormPreSimul" + args[0] + "," + args[1], players1);

        StdOut.println("Normal Input");
        StdOut.println("____________________");
        simulateInput(elo, postCalc, rounds);

        saveFreq("NormFreq" + args[0] + "," + args[1], elo.ratings(), postCalc.ratings());
        saveRS("NormRS" + args[0] + "," + args[1], elo, postCalc);

        Player[] players2 = maliciousInput(n);
        Elo elo2 = new Elo(players2);
        PostCalc postCalc2 = new PostCalc(players2);

        saveRS("MalPreSimul" + args[0] + "," + args[1], elo2, postCalc2);

        StdOut.println("\n\nMalicious Input");
        StdOut.println("____________________");
        simulateInput(elo2, postCalc2, rounds);

        saveFreq("MalFreq" + args[0] + "," + args[1], elo2.ratings(), postCalc2.ratings());
        saveRS("MalRS" + args[0] + "," + args[1], elo2, postCalc2);
    }
}
